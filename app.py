...
def index():
    if request.method == 'POST':
        ...
        queries = [
            ...
            f"What is the total revenue reported for quarter {quarter} in year {year}  ? Provide exact value only in the form - $xyz(dont use commas in between) millions always print millions, no extra information needed",
            f"What is the percentage of contribution of BFSI or Financial Segment or Sector  reported for quarter {quarter} in year {year}? Provide exact value only in the form - e.g. 20 %, no extra information needed. If no value found return 0.",
        ...
        csv_path = 'output/data4.csv'
        os.makedirs('output', exist_ok=True)
        
        if os.stat(csv_path).st_size == 0:  # File is empty
            with open(csv_path, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Name", "Quarter", "Year", "Total Revenue", "BFSI Percentile", "Date"])
